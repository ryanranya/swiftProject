...
//        setNeedsNavigationBarUpdate(titleColor: newTitleColor)
    ...
    {
        var displayLink:CADisplayLink? = CADisplayLink(target: self, selector: #selector(popNeedDisplay))
//        displayLink?.add(to: RunLoop.main, forMode: .Run)
        displayLink?.add(to: RunLoop.main, forMode: RunLoop.Mode.common)
        CATransaction.setCompletionBlock {
            displayLink?.invalidate()
            displayLink = nil
...
extension WRNavigationBar
{
    class func isIphoneX() -> Bool {
        return UIScreen.main.bounds.equalTo(CGRect(x: 0, y: 0, width: 375, height: 812)) || UIScreen.main.bounds.equalTo(CGRect(x: 0, y: 0, width: 414.0, height: 896.0))
    }
